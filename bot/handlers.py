...
def driver_payment_start(session: Session, user: User) -> None:
    ...
    set_state(session, user.vk_id, States.D_PAYMENT_TYPE)
...
def _payment_details_text(user: User) -> str:
    if user.payment_type == "card":
        ...
        details = f"телефон {user.payment_phone}, банк {user.payment_bank}"
    if user.payment_recipient:
        details += f", получатель {user.payment_recipient}"
    return details
...
def driver_send_payment_details(session: Session, user: User) -> None:
    ...
    vk.send_message(
        user.vk_id,
        "Реквизиты отправлены пассажиру.",
        keyboard=_driver_ride_kb(session, order),
    )
