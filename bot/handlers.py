...
def passenger_text_order(session: Session, user: User, text: str) -> None:
    ...
    if len(raw) < TEXT_ORDER_MIN_LENGTH or raw.casefold() in _menu_text_commands(session):
        return show_main_menu(session, user)
    labels = _passenger_labels(session)
...
def order_set_addresses(session: Session, user: User, text: str) -> None:
    ...
    # Recognize delivery wording and «Красный бархат» in normal, inflected,
    # abbreviated and frequently misspelled forms.
    is_delivery = delivery_service.is_delivery_text(raw)
    if is_delivery:
...
def disp_show_income(session: Session, user: User) -> None:
    """Show unpaid driver debts for today, yesterday and Monday–Sunday."""
    from . import dispatcher_report_service

    vk.send_message(
        user.vk_id,
        dispatcher_report_service.dispatcher_income_text(session, user.id),
        keyboard=kb.dispatcher_menu(can_switch_role(user)),
    )
