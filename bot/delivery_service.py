...
# Delivery recognition for free-form passenger requests.
# Besides the word «доставка», requests mentioning the «Красный бархат» shop
# are deliveries even when the passenger omits that word or changes word order.
_DELIVERY_PATTERNS = tuple(re.compile(pattern, re.IGNORECASE) for pattern in (
    # доставка / доставку / доставки / доставкой / доставить / доставьте
    r"(?<![а-яё])достав(?:к[а-яё]*|ить|ьте|ь)(?![а-яё])",
    # Красный бархат and grammatical forms: Красного бархата, Красном бархате.
    r"(?<![а-яё])красн(?:ый|ого|ому|ым|ом)\s+бархат(?:а|у|ом|е)?(?![а-яё])",
    # Frequent spelling typo: Красный бархот / Красного бархота.
    r"(?<![а-яё])красн(?:ый|ого|ому|ым|ом)\s+бархот(?:а|у|ом|е)?(?![а-яё])",
    # Short forms: «кр бархат», «кр. бархат», including inflections.
    r"(?<![а-яё])кр\.?\s+бархат(?:а|у|ом|е)?(?![а-яё])",
))


def is_delivery_text(text: str | None) -> bool:
    """Recognize delivery wording and variants of the «Красный бархат» name."""
    normalized = " ".join((text or "").split())
    return any(pattern.search(normalized) for pattern in _DELIVERY_PATTERNS)


def is_delivery(order: Order) -> bool:
    """Recognize both correctly typed and historically misclassified deliveries."""
    ...
    if is_delivery_text(request_text):
